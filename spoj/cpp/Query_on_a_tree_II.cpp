...
void solve()
{
    ...
    while (true)
    {
        ...
        else if (s[0] == 'K')
        {
            ...
            cin >> a >> b >> k;
            int lca = LCA(a, b);
            int lcaPos = level[a] - level[lca] + 1;

            // cout<<lcaPos<<endl;

            if (lcaPos >= k)    
            {
                cout << kAncestor(a, k - 1) << endl;
            }
            else
            {
                int pos = level[b] - level[lca] - (k - lcaPos);
                // cout<<pos<<endl;
                cout << kAncestor(b, pos) << endl;
            }
        }
        else
        {
            ...
            cin >> a >> b;
            int lca = LCA(a, b);
            cout << (queryHLD(a, lca) + queryHLD(b, lca)) << endl;
        }
    }
}
