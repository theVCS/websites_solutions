...
int level[2001], parent[2001][13], subSize[2001];
vector<pair<int, ll>> tree[2001];
pair<int, ll> specialChild[2001];
...
int timer, label[2001], chainHead[2001];
ll arr[2001];
...
void solve()
{
    ...
    REP(i, 1, n)
    {
        REP(j, i, n)
        {
            if (i < j)
            {
                ...
                m++;
                edg[m].a = i, edg[m].b = j, edg[m].w = dis[i][j];
    ...
    //         int a = i, b = j;
    //         int lca = LCA(a, b);
    //         ll c = (queryHLD(a, lca) + queryHLD(b, lca));
    //         if (dis[i][j] != c)
    //         {
    //             cout << "NO";
    //             return;
