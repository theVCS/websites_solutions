...
struct process
{
    int p, at, bt, prior, abt, st, ct, wt, tat, rt;
...
public:
    ...
    void runProcess()
    {
        ...
        while (!q.empty())
        {
            ...
            // checking if having same priority

            if (!q.empty() && q.top().first == p.first && pro[q.top().second].at <= currTime)
            {
                readyQue.push(p.second);

                while (!q.empty() && q.top().first == p.first && pro[q.top().second].at <= currTime)
                {
